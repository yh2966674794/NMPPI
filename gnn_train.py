...
from gcn_model import GCN_Net2
from gnn_model_pre import GIN_Net2_pre
from utils import Metrictor_PPI, print_file
from sklearn.metrics.pairwise import cosine_similarity
import torch.nn.functional as F
from cl_model import CL_NET
...
def get_edgeindex(proteins, protein_edge_index, mask, threshold):
    node_id = protein_edge_index[:, mask]
    x1 = proteins[node_id[0]]
    x2 = proteins[node_id[1]]
    x_ppi = x1 * x2
    x_ppi = x_ppi.detach().cpu().numpy()
    sims = cosine_similarity(x_ppi.reshape(x_ppi.shape[0], -1))
    edges = []
    ...
    for i in range(len(edges)):
        temp_ppi = edges[i][::-1]
        edges.append(temp_ppi)
    ppi_edge = np.array(edges)
    return ppi_edge


def get_edge_ppi(x_ppi, threshold):
    edges = []
    for i in range(x_ppi.shape[0]):
        for j in range(i + 1, x_ppi.shape[0]):
            cos_sim = x_ppi[i].dot(x_ppi[j]) / (np.linalg.norm(x_ppi[i]) * np.linalg.norm(x_ppi[j]))
...
def n2v_get_struct(edge_list, type, split_new):
    if split_new:
        G = nx.Graph()
        G.add_edges_from(edge_list)
        node2vec = Node2Vec(G, dimensions=256, walk_length=80, num_walks=10,
                            workers=4)  # Use temp_folder for big graphs

        model_n2v = node2vec.fit(window=10, min_count=1, batch_words=4, epochs=50)
        model_n2v.wv.save_word2vec_format("embedding" + "_" + type)
    embeddings_dict = {}
    embeddings = open("embedding" + "_" + type).read().splitlines()[1:]
    embeddings = [x.split(" ") for x in embeddings]
    for i in range(0, len(embeddings)):
        key = int(embeddings[i][0])
        embeddings_dict[key] = [float(x) for x in embeddings[i][1:]]

    embeddings_list = [[0 for i in range(256)]] * 1690
    for embedding in embeddings_dict.keys():
        embeddings_list[embedding] = embeddings_dict[embedding]
    struct_x = np.array(embeddings_list)
    struct_x = torch.tensor(struct_x, dtype=torch.float)
    return struct_x


def train(model, graph, ppi_list, loss_fn, optimizer, device,
          result_file_path, summary_writer, save_path,
          batch_size=512, epochs=200, scheduler=None,
          got=False):
    global_step = 0
    ...
    for epoch in range(epochs):
        ...
        print_file("epoch: {}, Training_avg: label_loss: {}, recall: {}, precision: {}, F1: {}, Validation_avg: loss: "
                   "{}, recall: {}, precision: {}, F1: {}, Best valid_f1: {}, in {} epoch"
...
def main():
    ...
    ppi_data.get_protein_emb(pemb_path=args.pseq_path)
